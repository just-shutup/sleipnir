...
Известные рамки: сигнатурный детект (версию, скрытую в баннере, не матчит), JS-рендеринг требует отдельного headless-браузера, SSRF/IDOR остаются потенциальными без внешнего canary или второй учётной записи, UDP-скан ведёт себя как `open|filtered` на молчащих портах (без активных RPC-проб), SYN-скан требует Linux и root/CAP_NET_RAW (иначе автоматический fallback на connect), SMB/RPC не пробируются. Дорожная карта: master/worker-раздача целей по сети, фазовый движок с resume, TLS-аудит всех портов, экспорт SARIF, diff между запусками.

## Проект

Sleipnir v0.9.0 — это самостоятельный проект по информационной безопасности: асинхронный сетевой движок на Asio, интеграция Lua (sol2), декларативные базы знаний с активной CVE-верификацией, authenticated web scanning (`--auth`), robots/sitemap/JS endpoint discovery, dirb-сканирование (`--dirb`), параметрические проверки SQLi/SSTI/SSRF/XXE/IDOR и TLS/ALPN-инспекция. Найденные проблемы разделяются на `confirmed` и `potential` с доказательством в `evidence`; база знаний обновляется через `sleipnir db update`.
